<?php
/*
Plugin Name: Bosa Elementor Addons for WooCommerce
...
if (!class_exists('BEW')) {
    class BEW{
        ...
        public function __construct() {
            ...
            }else if ( !class_exists( 'WooCommerce', false ) ) {
         ...
         * @since Bosa Elementor Addons for WooCommerce 1.0.1
         */
        ...
        function admin_notice__error_ele() {
            ...
            if ( $this->_is_plugin_installed( $elementor ) ) {
                ...
                $message = sprintf( __('%1$sBosa Elementor Addons for WooCommerce%2$s requires %1$sElementor%2$s plugin to be active. Please activate Elementor to continue.', 'bosa-elementor-for-woocommerce'), "<strong>", "</strong>");
    
                $button_text = __('Activate Elementor', 'bosa-elementor-for-woocommerce');
            } else {
                $activation_url = wp_nonce_url(self_admin_url('update.php?action=install-plugin&plugin=elementor'), 'install-plugin_elementor');
    
                $message = sprintf(__('%1$sBosa Elementor Addons for WooCommerce%2$s requires %1$sElementor%2$s plugin to be installed and activated. Please install Elementor to continue.', 'bosa-elementor-for-woocommerce'), '<strong>', '</strong>');
        ...
        function admin_notice__error_woo() {
            ...
            if ( $this->_is_plugin_installed( $elementor ) ) {
                ...
                $message = sprintf( __('%1$sBosa Elementor Addons for WooCommerce%2$s requires %1$sWooCommerce%2$s plugin to be active. Please activate WooCommerce to continue.', 'bosa-elementor-for-woocommerce'), "<strong>", "</strong>");
    
                $button_text = __('Activate WooCommerce', 'bosa-elementor-for-woocommerce');
            } else {
                $activation_url = wp_nonce_url(self_admin_url('update.php?action=install-plugin&plugin=elementor'), 'install-plugin_elementor');
    
                $message = sprintf(__('%1$sBosa Elementor Addons for WooCommerce%2$s requires %1$sWooCommerce%2$s plugin to be installed and activated. Please install WooCommerce to continue.', 'bosa-elementor-for-woocommerce'), '<strong>', '</strong>');
         ...
         *
         * @since Bosa Elementor Addons for WooCommerce
         */
